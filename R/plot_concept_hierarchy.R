...
plot_concept_hierarchy <-
    function(concept_id, generations_before = 2, generations_after = 2) {
        
        output_descendents <- list()
        df_a <-
        ...
        output_descendents[[1]] <-
            dplyr::left_join(df_a,
                             df_b)
        
        for (i in 1:generations_after) {
            previous_output <- output_descendents[[i]]
            ...
            output_descendents[[next_output_index]] <- dplyr::left_join(df_a,
                                                            df_b) %>%
                dplyr::distinct()
            
        }
        output_descendents <- dplyr::bind_rows(output_descendents)
